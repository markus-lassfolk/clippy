...
PRs also get an **lcov** comment from `romeovs/lcov-reporter-action` when
...
- [Actions tab](https://github.com/markus-lassfolk/m365-agent-cli/actions)
  — green = latest `main` / PR checks passed.
...
- **Knip** is configured in [`knip.json`](../knip.json);
  some public API surfaces are ignored to avoid false positives.
- **TruffleHog** uses `continue-on-error: true` (verified hits only);
  **Gitleaks** remains the strict gate for secret patterns
  in the repo.
