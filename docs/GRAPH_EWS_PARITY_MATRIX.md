...
**Related:** [`MIGRATION_TRACKING.md`](./MIGRATION_TRACKING.md) (migration status), [`exchange-backend.ts`](../src/lib/exchange-backend.ts) (mode semantics), [`GRAPH_SCOPES.md`](./GRAPH_SCOPES.md) (OAuth scopes).
...
| `delegates` **add/update/remove** | **Error** — set `M365_EXCHANGE_BACKEND=ews` or **`auto`** | **EWS** (`requireEwsForDelegateMutations` blocks **only** when mode is `graph`) | EWS | **No Graph API** for classic delegate matrix; **`auto`** runs the EWS SOAP path. |
