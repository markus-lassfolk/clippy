...
describe('copilot-graph-client build helpers', () => {
  it('buildCopilotRetrievalBody validates query and dataSource', () => {
    expect(() => buildCopilotRetrievalBody({ queryString: '', dataSource: 'sharePoint' })).toThrow(GraphApiError);
    const long = 'x'.repeat(1600);
    expect(() => buildCopilotRetrievalBody({ queryString: long, dataSource: 'sharePoint' })).toThrow(GraphApiError);
    expect(() => buildCopilotRetrievalBody({ queryString: 'ok', dataSource: 'bad' })).toThrow(GraphApiError);
    const b = buildCopilotRetrievalBody({
      queryString: ' q ',
    ...
    expect(b.queryString).toBe('q');
    expect(b.dataSource).toBe('oneDriveBusiness');
    expect(b.filterExpression).toBe('f');
    expect(b.maximumNumberOfResults).toBe(10);
    expect(b.resourceMetadata).toEqual(['a']);
    expect(() =>
      buildCopilotRetrievalBody({ queryString: 'ok', dataSource: 'externalItem', maximumNumberOfResults: 0 })
    ).toThrow(GraphApiError);
    expect(() =>
      buildCopilotRetrievalBody({ queryString: 'ok', dataSource: 'externalItem', maximumNumberOfResults: 99 })
    ).toThrow(GraphApiError);
    expect(() =>
      buildCopilotRetrievalBody({ queryString: 'ok', dataSource: 'externalItem', maximumNumberOfResults: NaN })
    ).toThrow(GraphApiError);

    expect(() => buildCopilotSearchBody({ query: '' })).toThrow(GraphApiError);
    const longQ = 'y'.repeat(1600);
    expect(() => buildCopilotSearchBody({ query: longQ })).toThrow(GraphApiError);
    expect(() => buildCopilotSearchBody({ query: 'ok', pageSize: 0 })).toThrow(GraphApiError);
    expect(() => buildCopilotSearchBody({ query: 'ok', pageSize: 101 })).toThrow(GraphApiError);
    const minimal = buildCopilotSearchBody({ query: ' hello ' });
    expect(minimal.query).toBe('hello');
    const full = buildCopilotSearchBody({
      query: 'q',
      pageSize: 50,
      oneDriveFilterExpression: ' f ',
      resourceMetadataNames: ['', 'meta', '']
    });
    expect(full.pageSize).toBe(50);
    expect((full.dataSources as { oneDrive: Record<string, unknown> }).oneDrive.filterExpression).toBe('f');
    expect(
      (full.dataSources as { oneDrive: { resourceMetadataNames: string[] } }).oneDrive.resourceMetadataNames
    ).toEqual(['meta']);

    for (const p of COPILOT_REPORT_PERIODS) {
      expect(assertCopilotReportPeriod(` ${p} `)).toBe(p);
      for (const fn of [
        'getMicrosoft365CopilotUserCountSummary',
        'getMicrosoft365CopilotUserCountTrend',
        'getMicrosoft365CopilotUsageUserDetail'
      ] as const) {
        const path = copilotReportPath(fn, p);
        expect(path).toContain(`period='${p}'`);
        expect(path).toContain('$format=application/json');
      }
    }
    expect(() => assertCopilotReportPeriod('bad')).toThrow(GraphApiError);
    expect(() => copilotReportPath('getMicrosoft365CopilotUserCountSummary', 'bad')).toThrow(GraphApiError);

    expect(() => copilotUserSegment('')).toThrow(GraphApiError);
    expect(copilotUserSegment(' u@x.com ')).toContain('%40');
    expect(appendCopilotODataQuery('/p', undefined)).toBe('/p');
    expect(appendCopilotODataQuery('/p', '   ')).toBe('/p');
    expect(appendCopilotODataQuery('/p', '$top=1')).toBe('/p?$top=1');
    expect(appendCopilotODataQuery('/p?', '?$skip=1')).toBe('/p?&$skip=1');
    expect(appendCopilotODataQuery('/p?a=1', 'b=2')).toBe('/p?a=1&b=2');
  });
});
