...
import {
  ...
  createTask, updateTask, deleteTask, addChecklistItem,
  type TodoImportance, type TodoStatus, type TodoTask, type TodoList
...
function fmtDT(d: { dateTime: string; timeZone: string } | undefined): string {
  ...
  try {
    return new Date(d.dateTime).toLocaleString("en-US", {
      timeZone: d.timeZone || "UTC", month: "short", day: "numeric", year: "numeric",
      hour: "2-digit", minute: "2-digit", hour12: false
    });
  } catch { return d.dateTime; }
}

function impEmoji(i: TodoImportance | undefined): string { return i === "high" ? "\u{1F534}" : i === "low" ? "\u{1F535}" : "\u26AA"; }
function stsEmoji(s: TodoStatus | undefined): string {
  switch (s) {
    case "completed":        return "\u2705";
    case "inProgress":       return "\u{1F504}";
    case "waitingOnOthers":  return "\u23F3";
    case "deferred":        return "\u{1F4E6}";
...
todoCommand
  ...
  .action(async (opts: { json?: boolean; token?: string }) => {
    ...
    if (!auth.success) { console.error(`Auth error: ${auth.error}`); process.exit(1); }
    ...
    if (lists.length === 0) { console.log("No task lists found."); return; }
...
todoCommand
  ...
  .option("--token <token>", "Use a specific token")
  .action(async (opts: {
    list?: string; task?: string; status?: string; importance?: string;
    ...
    if (!listsR.ok || !listsR.data) { console.error(`Error: ${listsR.error?.message}`); process.exit(1); }

    const matched = listsR.data.find(
      (l) => l.id === listName || l.displayName.toLowerCase() === listName.toLowerCase() ||
             l.wellknownListName?.toLowerCase() === listName.toLowerCase()
    );

    let listId: string, listDisplay: string;
    if (matched) { listId = matched.id; listDisplay = matched.displayName; }
    else {
      const s = await getTodoList(auth.token!, listName);
      if (!s.ok || !s.data) { console.error(`List not found: "${listName}".`); console.error('Use "clippy todo lists".'); process.exit(1); }
      listId = s.data.id; listDisplay = s.data.displayName;
    }

    if (opts.task) {
      const r = await getTask(auth.token!, listId, opts.task);
      if (!r.ok || !r.data) { console.error(`Error: ${r.error?.message}`); process.exit(1); }
      ...
      if (t.body?.content) { console.log(`\n${hr}\n${t.body.content}`); }
      if (t.checklistItems?.length) {
        console.log("\nChecklist:");
        for (const item of t.checklistItems) console.log(`  ${item.isChecked ? "\u2611" : "\u2610"} ${item.displayName}`);
      }
      console.log(`\n${hr}`);
      console.log(`ID:          ${t.id}`);
    ...
    if (opts.json) { console.log(JSON.stringify({ list: listDisplay, listId, tasks }, null, 2)); return; }
    ...
    for (const t of tasks) {
      ...
      if (t.linkedResources?.length) console.log(`      \u21B3 linked: ${t.linkedResources.map((l) => l.description).join(", ")}`);
      console.log("");
    }
  });
...
todoCommand
  ...
  .option("--token <token>", "Use a specific token")
  .action(async (opts: {
    title: string; list?: string; body?: string; due?: string;
    importance?: string; status?: string; reminder?: string; link?: string;
    json?: boolean; token?: string;
  }) => {
    const auth = await resolveGraphAuth({ token: opts.token });
    if (!auth.success) { console.error(`Auth error: ${auth.error}`); process.exit(1); }
    ...
    const result = await createTask(auth.token!, listId, {
      ...
      dueDateTime: opts.due, reminderDateTime: opts.reminder,
      isReminderOn: !!opts.reminder, linkedResources
    });
    if (!result.ok || !result.data) { console.error(`Error: ${result.error?.message}`); process.exit(1); }
...
todoCommand
  ...
  .action(async (opts: { list: string; task: string; json?: boolean; token?: string }) => {
    ...
    if (!listsR.ok || !listsR.data) { console.error(`Error: ${listsR.error?.message}`); process.exit(1); }
    const matched = listsR.data.find(
      (l) => l.id === opts.list || l.displayName.toLowerCase() === opts.list.toLowerCase() ||
             l.wellknownListName?.toLowerCase() === opts.list.toLowerCase()
    );
    const listId = matched?.id || opts.list;
...
todoCommand
  ...
  .action(async (opts: { list: string; task: string; confirm?: boolean; token?: string }) => {
    ...
    const listId = matched?.id || opts.list;
    const listName = matched?.displayName || opts.list;
    const taskR = await getTask(auth.token!, listId, opts.task);
    if (!taskR.ok || !taskR.data) { console.error(`Task not found: ${taskR.error?.message}`); process.exit(1); }
    if (!opts.confirm) { console.log(`Delete "${taskR.data.title}" from "${listName}"? (ID: ${opts.task})`); console.log("Run with --confirm to confirm."); process.exit(1); }
